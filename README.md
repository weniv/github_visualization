...
```
1. 명령어 기록 ui 변경하여 가독성을 높이자는 의견이 나왔습니다.
2. 기록에 sorting 기능을 추가하여 원하는 기록을 볼 수 있도록 하자는 의견이 나왔습니다.
3. 기록의 경우 JSON 형태로 저장하여 파일 다운로드 시 JSON 파일로 다운로드 하자는 의견이 나왔습니다.
```

- 8월 28일

```
1. 명령어 버튼 hover 시에 나오는 메시지는 구현하지 않기로 하였습니다.
2. 튜토리얼(코치마크)의 경우, 다음 및 스킵 기능을 추가하기로 하였습니다.
```
